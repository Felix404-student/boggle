...
class Boggle():
    ...
    def find_from(self, board, word, y, x):
        ...
        # checking above, below, left, right 
        if y > 0:
            if self.find_from(board, word[1:], y - 1, x):
                return True
        ...
        if x > 0:
            if self.find_from(board, word[1:], y, x - 1):
                return True
        ...
        # diagonals
        if y > 0 and x > 0:
            if self.find_from(board, word[1:], y - 1, x - 1):
                return True
